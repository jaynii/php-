...
if ($act == "Del") {
    $sql = "delete from mianmao where id=$id";
    $result = mysql_query($sql);

}
if ($id != "") {
    $sql = "select * from mianmao where id=$id";
    $result = mysql_query($sql);
    ...
    <tr>
        <td>
            <table width="500" border="0" align="center" cellpadding="0" cellspacing="1">
                <?php
                $sql = "select * from mianmao";
                $result = mysql_query($sql);
                ?>
                <tr align="center">
                    <td bgcolor="#FFFFFF">政治面貌名称</td>
...
if ($act == "save") {
    if ($id == "") {
        $sql = "insert into mianmao (name) values ('$p0')";
        if (mysql_query($sql)) {
            echo "<script>location.href='mianmao.php'</script>";
            exit;
        } else {
            echo "添加失败";
            exit;
    ...
    $sql = "update mianmao set name='$p0' where id=$id";
    mysql_query($sql);
    echo "<script>location.href='mianmao.php'</script>";
    exit;
}
...
<table width="750" border="0" align="center" cellpadding="0" cellspacing="0">
    <form name="form1" method="post" action="?act=save&id=<?php echo $data[id] ?>" onSubmit="return chkinput(this)">
        <tr>
            <td height="30">
                <div align="center">添加/编辑政治面貌</div>
            </td>
        </tr>
        <tr>
            <td>
                <table width="580" border="0" align="center" cellpadding="0" cellspacing="1">
                    <tr>
                        <td height="25" bgcolor="#FFFFFF">
                            <div align="right">政治面貌名称</div>
                        </td>
                        <td bgcolor="#FFFFFF">
                            <div align="left"><input type="text" name="p0" class="inputcss" size="20"
                                                     value="<?php echo $data[name] ?>"></div>
                        </td>
                    </tr>
                </table>
            </td>
