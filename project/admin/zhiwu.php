...
if ($act == "Del") {
    $sql = "delete from zhiwu where id=$id";
    $result = mysql_query($sql);
}
if ($id != "") {
    $sql = "select * from zhiwu where id=$id";
    $result = mysql_query($sql);
...
if ($act == "save") {
    if ($id == "") {
        $sql = "insert into zhiwu (name) values ('$p0')";
        if (mysql_query($sql)) {
            echo "<script>location.href='zhiwu.php'</script>";
            exit;
        } else {
            echo "添加失败";
            exit;
    ...
    $sql = "update zhiwu set name='$p0' where id=$id";
    mysql_query($sql);
    echo "<script>location.href='zhiwu.php'</script>";
    exit;
}
